...
func main() {
	...
	args := flag.Args()
	blockStoreAddrList := []string{}
	for _, arg := range args {
		blockStoreAddrList = append(blockStoreAddrList, arg)
	}
	...
	log.Fatal(startServer(addr, strings.ToLower(*service), blockStoreAddrList))
}

func startServer(hostAddr string, serviceType string, blockStoreAddrList []string) error {
	...
	if serviceType == "meta" {
		metaStore := surfstore.NewMetaStore(blockStoreAddrList)
